...
CREATE TABLE BAKERY.devices (
     ...
     , device_recip_id INTEGER NOT NULL
     , PRIMARY KEY (device_id)
     , INDEX (device_recip_id)
     , CONSTRAINT FK_devices_1 FOREIGN KEY (device_recip_id)
...
CREATE TABLE BAKERY.device_parameters (
     ...
     , device_parameter_changable CHAR(10)
...
CREATE TABLE BAKERY.recip_parameters (
       recip_parameter_id INTEGER NOT NULL
     , device_parameter_id INTEGER NOT NULL
     , INDEX (recip_parameter_id)
     , CONSTRAINT FK_recip_parameters_1 FOREIGN KEY (recip_parameter_id)
                  REFERENCES BAKERY.recips (recip_id)
...
CREATE TABLE BAKERY.plans (
     ...
     , CONSTRAINT FK_plans_1 FOREIGN KEY (order_id)
                  REFERENCES BAKERY.orders (order_id)
     , INDEX (device_id)
     , CONSTRAINT FK_plans_2 FOREIGN KEY (device_id)
                  REFERENCES BAKERY.devices (device_id)
);
