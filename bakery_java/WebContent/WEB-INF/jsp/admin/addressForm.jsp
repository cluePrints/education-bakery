...
<form id="actionForm" method="post">
	<table class="content">
		...
		<%-- Main content --%>
				<spring:bind path="addressFormBean">		
			...
			<spring:nestedPath path="addressFormBean">
			<%@include file="/WEB-INF/jsp/common/paginator.jsp" %>
			</spring:nestedPath>
			<table>
				<tr class="tableHead">
				<td><u><fmt:message key="addresses.table.id.caption" bundle="${resourceBundle}"/></u></td>
				<td><u><fmt:message key="addresses.table.name.caption" bundle="${resourceBundle}"/></u></td>
				<td><u><fmt:message key="addresses.table.actions.caption" bundle="${resourceBundle}"/></u></td></tr>
				<%-- Iteration over items--%>
				<c:forEach items="${addresses}" var="address">
					<%-- if is active--%>
						<c:if test="${address.active>0}">		
						...
						<c:if test="${address.active<=0}">		
							<tr style='<%=org.bakery.server.util.Constants.STYLE_INACTIVE%>'>
								...
								<td><c:out value="${address}" escapeXml="false"/></td>
								<td><a href="javascript:<c:if test="${address.active>0}">de</c:if>activate(<c:out value="${address.id}"/>)">${address.active<=0 ? 'Restore' : 'Remove'}</a><BR>
									<a href="javascript:showEdit(${address.id},'${address.name}', ${address.active>0 ? 'true' : 'false'})">Edit</a><BR></td></tr>				
				</c:forEach>
				<%-- /Iteration --%>
			</table>			
		<%-- /main content--%>
